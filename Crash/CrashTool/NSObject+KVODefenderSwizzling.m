//
//  NSObject+KVODefenderSwizzling.m
//  Crash
//
//  Created by dzc on 2021/1/30.
//

#import "NSObject+KVODefenderSwizzling.h"
...
#pragma mark - NSObject+KVODefenderSwizzling 分类

@implementation NSObject (KVODefenderSwizzling)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [NSObject baseSwizzlingInstanceMethod: @selector(addObserver:forKeyPath:options:context:)
                                          withMethod: @selector(xxx_addObserver:forKeyPath:options:context:)
                                           withClass: [NSObject class]];
        
        // 拦截 `removeObserver:forKeyPath:` 方法，替换自定义实现
        [NSObject baseSwizzlingInstanceMethod: @selector(removeObserver:forKeyPath:)
                                          withMethod: @selector(xxx_removeObserver:forKeyPath:)
                                           withClass: [NSObject class]];
        
        // 拦截 `removeObserver:forKeyPath:context:` 方法，替换自定义实现
        [NSObject baseSwizzlingInstanceMethod: @selector(removeObserver:forKeyPath:context:)
                                          withMethod: @selector(xxx_removeObserver:forKeyPath:context:)
                                           withClass: [NSObject class]];
        
        // 拦截 `dealloc` 方法，替换自定义实现
        [NSObject baseSwizzlingInstanceMethod: NSSelectorFromString(@"dealloc")
                                          withMethod: @selector(xxx_kvodealloc)
                                           withClass: [NSObject class]];
    });
}
...
- (void)xxx_addObserver:(NSObject *)observer
             forKeyPath:(NSString *)keyPath
                options:(NSKeyValueObservingOptions)options
                context:(void *)context {
    ...
    if (!isSystemClass(self.class)) {
        ...
        if ([self.yscKVOProxy addInfoToMapWithObserver:observer forKeyPath:keyPath options:options context:context]) {
            ...
            [self xxx_addObserver:self.yscKVOProxy forKeyPath:keyPath options:options context:context];
        } else {
            // 添加 KVO 信息操作失败：重复添加
        ...
        [self xxx_addObserver:observer forKeyPath:keyPath options:options context:context];
    }
}

// 自定义 removeObserver:forKeyPath:context: 实现方法
- (void)xxx_removeObserver:(NSObject *)observer
                forKeyPath:(NSString *)keyPath
                   context:(void *)context {
    ...
    if (!isSystemClass(self.class)) {
        if ([self.yscKVOProxy removeInfoInMapWithObserver:observer forKeyPath:keyPath context:context]) {
            ...
            [self xxx_removeObserver:self.yscKVOProxy forKeyPath:keyPath context:context];
        } else {
        ...
        [self xxx_removeObserver:observer forKeyPath:keyPath context:context];
    }
}
...
- (void)xxx_removeObserver:(NSObject *)observer
    ...
    if (!isSystemClass(self.class)) {
        if ([self.yscKVOProxy removeInfoInMapWithObserver:observer forKeyPath:keyPath]) {
            // 如果移除 KVO 信息操作成功，则调用系统移除方法
            [self xxx_removeObserver:self.yscKVOProxy forKeyPath:keyPath];
        } else {
            // 移除 KVO 信息操作失败：移除了未注册的观察者
            NSString *className = NSStringFromClass(self.class) == nil ? @"" : NSStringFromClass(self.class);
            NSString *reason = [NSString stringWithFormat:@"*** Crash Message: Cannot remove an observer %@ for the key path '%@' from %@ , because it is not registered as an observer ***", observer, keyPath, className];
        ...
        [self xxx_removeObserver:observer forKeyPath:keyPath];
    }
